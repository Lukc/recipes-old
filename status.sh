...
subtitle() {
	if [[ "$OUTPUT" = html ]]; then
		...
		echo # The empty line never hurts.
		echo "-= {{ $@ }} =-"
	fi
}
...
function test_general {
	...
	if [[ -z "${users[@]}" ]]; then
		users=(Youbi Kooda Lukc Pingax)
	fi
	
	for packager in ${users[@]}; do
		subtitle "$packager"
		: > log.$packager
		for i in *; do
			(
...
function test_lastvers {
	subtitle "bad lastvers"
	...
	for DIR in *; do
		...
		(
			...
			subsubtitle "$DIR"
			if [[ -n "$lastver" ]]; then
				if [[ "$lastver" =~ "lynx -dump" ]]; then
					error "lynx -dump is used."
				fi
			else
				error "No \$lastver."
			fi
...
if [[ -z "${tests[@]}" ]]; then
	tests=(general orphans useflags lastvers)
fi
